...
import { DEFAULT_BRAND_NAME, slugifyBrandName } from "@/lib/workspace/tenant";

export async function GET(request: Request) {
  try {
    const brandId = request.headers.get("x-distributor-os-brand-id") || PILOT_BRAND_ID;
...
export async function POST(request: Request) {
  try {
    ...
    const brandId = body.brand_id || PILOT_BRAND_ID;
    const brandName = body.brand_name || DEFAULT_BRAND_NAME;
